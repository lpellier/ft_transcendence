...
export const Sketch = (p: any) => {
  ...
  class Buttons {
    ...
    reset() {
      if (this.create_game) this.create_game.remove();
  ...
  function listenStartEvents() {
    socket.on(
      ...
      (r_id: string, score_limit: number, map: string) => {
        ...
        else if (map === "casino") game.map = consts.casino_map;
    ...
    socket.on("matchmaking-error", (error: string) => {
      errors.set_false();
      if (error === "game_full") errors.game_full = true;
      else if (error === "game_not_found") errors.game_not_found = true;
  ...
  p.draw = () => {
    ...
    if (!document.getElementById("canvas-parent")) {
      ...
      return;
    } else if (should_load) inMainMenu();
    if (
      game.state === "waiting-player" ||
      game.state === "waiting-readiness" ||
      game.state === "countdown" ||
